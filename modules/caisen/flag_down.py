...
class FlagDownDetector(BasePatternDetector):
    ...
    def _check_window(self, window: list, kline_df, global_start: int = 0) -> "PatternResult | None":
        ...
        if last_close > upper_line:
            validation = self.validator.validate(
                kline_df, breakout_date, upper_line, direction="bull"
            )
...
def _calc_slope(points: list) -> float:
    if len(points) < 2:
        return 0.0
    ...
    try:
        first_date = pd.Timestamp(points[0][0])
        last_date = pd.Timestamp(points[-1][0])
        days = max((last_date - first_date).days, 1)
        price_diff = points[-1][1] - points[0][1]
        return price_diff / days
    except Exception:
        return 0.0
